...
;
(defun amocHelper1 (lst x)
	(cond	((null lst) x)
		(t (amocHelper1 (rest lst) (append x (list (- (first lst))))))
  	)
)

(defun amocHelper2 (n c m k x)
	(cond	((= c k) x)
    		(t (amocHelper2 n c m (- k 1) (append (list (amocHelper1 (list (node2var n c m) (node2var n k m)) ())) x)))
  	)
)

(defun amocHelper (n c k x)
	(cond	((> c k) x)
    		(t (amocHelper n (+ c 1) k (append x (amocHelper2 n c k k ()))))
  	)
)
...
;
(defun gecHelper (x y z k)
	(cond 	((> z k) ())
		(t (append (list (list (- (node2var x z k)) (- (node2var y z k)))) (gecHelper x y (+ z 1) k)))
	)
)
